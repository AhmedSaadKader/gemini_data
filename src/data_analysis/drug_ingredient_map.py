...
class BatchProcessor:
    ...
    def create_prompt(self, compound: str, known_ingredients: Set[str]) -> str:
        ...
        return f'''You are a pharmaceutical database expert. Analyze this drug compound and map it to known ingredients.

Input compound: "{compound}"

Known active ingredients (subset): {", ".join(known_sample)}
    ...
    def parse_response(self, response: str) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Attempted to parse: {response}")
                raise
            ...
            # Validate mappings
            valid_mappings = []
            required_keys = {'original', 'mapped_ingredient', 'confidence', 'notes'}
            
            for idx, mapping in enumerate(mappings):
                if not isinstance(mapping, dict):
                    self.logger.error(f"Mapping {idx} is not a dictionary")
                    continue
                    
                # Check for required keys
                if not all(key in mapping for key in required_keys):
                    missing_keys = required_keys - set(mapping.keys())
                    self.logger.error(f"Mapping {idx} missing keys: {missing_keys}")
                    continue
                ...
                try:
                    ...
                    if not 0 <= confidence <= 1:
                        self.logger.error(f"Mapping {idx} has invalid confidence: {confidence}")
                        continue
                    mapping['confidence'] = confidence
                except (ValueError, TypeError):
                    self.logger.error(f"Mapping {idx} has non-numeric confidence: {mapping['confidence']}")
                    continue
                
                # Validate strings
                if not all(isinstance(mapping[k], str) for k in ['original', 'mapped_ingredient', 'notes']):
                    self.logger.error(f"Mapping {idx} has non-string values for text fields")
                    continue
                
                valid_mappings.append(mapping)
            
            self.logger.info(f"Validated {len(valid_mappings)} of {len(mappings)} mappings")
